        ...
        {item?.data?.sectionId === "h100" ? (
          <div className="flex flex-col gap-1">
            ...
            <span>
              ...
              <span>{item?.item?.artists[1]?.name}, </span>
              <span>{item?.item?.artists[2]?.name}</span>
              <span>...</span>
            </span>
          </div>
        ) : item?.data?.sectionId === "hAlbum" ? (
          <div className="flex flex-col gap-1">
            ...
            <span>{item?.item?.artistsNames}</span>
          </div>
        ) : (
          <span>{`${
            item?.item.sortDescription?.length > 55
              ? `${item?.item.sortDescription?.slice(0, 55)}...`
              : item?.item.sortDescription
          }`}</span>
        )}
      </span>
    </div>
